...
def _apply_skill_effect(performer: Monster, target: Monster, skill: Skill, game_configs: GameConfigs) -> Dict[str, Any]:
    ...
    if skill.get("effect"):
        if skill.get("effect") == "heal" and skill.get("amount"):
            ...
            performer["current_hp"] = min(performer.get("hp", 0) + performer.get("cultivation_gains", {}).get("hp",0), performer.get("current_hp", performer.get("hp", 0)) + heal_amount)
...
def _process_health_conditions(monster: Monster) -> Tuple[bool, List[str]]:
    ...
    for condition in monster["healthConditions"]:
        ...
        if "hp_per_turn" in condition.get("effects", {}):
            ...
            monster["current_hp"] = max(0, min(monster.get("hp", 0) + monster.get("cultivation_gains",{}).get("hp",0), monster.get("current_hp", 0))) # 確保不超過最大生命值且不低於0
        ...
        if condition.get("confusion_chance", 0) > 0 and random.random() < condition.get("confusion_chance", 0):
            confusion_damage = int(monster.get("attack", 0) * 0.1) # 簡化為攻擊力10%的自傷
...
def simulate_battle_full(
    ...
    # 初始化當前 HP/MP
    player_monster["current_hp"] = player_monster.get("current_hp", player_monster.get("hp", 0))
    player_monster["current_mp"] = player_monster.get("current_mp", player_monster.get("mp", 0))
    opponent_monster["current_hp"] = opponent_monster.get("current_hp", opponent_monster.get("hp", 0))
    opponent_monster["current_mp"] = opponent_monster.get("current_mp", opponent_monster.get("mp", 0))
    ...
    for turn_num in range(1, max_turns + 2): # 加 1 處理最終回判斷平手
        ...
        if player_monster.get("current_hp", 0) <= 0 or opponent_monster.get("current_hp", 0) <= 0:
            ...
            for i, current_actor in enumerate(acting_order):
                ...
                if (current_actor.get("id") == player_monster.get("id") and player_skip) or \
                   (current_actor.get("id") == opponent_monster.get("id") and opponent_skip):
                ...
                if chosen_skill:
                    ...
                    # 更新戰鬥統計
                    is_player_turn = current_actor.get("id") == player_monster.get("id")
                    ...
                    all_turn_actions.append(BattleAction( # type: ignore
                        performer_id=current_actor.get("id", ""), target_id=target_actor.get("id", ""), skill_name=chosen_skill.get("name", ""),
                        ...
                        is_miss=action_result.get("is_miss"), log_message=action_result.get("log_message", "")
                    ))
                else:
    ...
    if winner_id == player_monster.get('id'): # 玩家勝利
    ...
    elif winner_id == opponent_monster.get('id'): # 玩家失敗
    ...
    final_battle_result: BattleResult = {
        ...
        "battle_highlights": battle_highlights, # type: ignore # 新增戰鬥亮點
