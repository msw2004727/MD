...
def get_friends_status_service(friend_ids: List[str]) -> List[Dict[str, Any]]:
    """
    根據好友 UID 列表，批量查詢他們最新的上線時間和暱稱。
    """
    if not MD_firebase_config.db:
        leaderboard_search_services_logger.error("Firestore 資料庫未初始化 (get_friends_status_service 內部)。")
        return []

    db = MD_firebase_config.db
    friend_statuses: List[Dict[str, Any]] = []

    if not friend_ids:
        return []

    try:
        # Firestore 的 in 查詢最多支援 10 個元素
        # 如果 friend_ids 數量可能超過 10，需要分批查詢
        for i in range(0, len(friend_ids), 10):
            batch_ids = friend_ids[i:i + 10]
            users_ref = db.collection('users')
            
            # 使用 where(filter=...) 替代 where() 進行複雜查詢
            docs = users_ref.where(filter=firestore.FieldFilter(firestore.FieldPath.document_id(), 'in', batch_ids)).stream()
            ...
            for doc in docs:
                ...
                if user_data:
                    friend_statuses.append({
                        "uid": doc.id,
                        "nickname": user_data.get("nickname", "未知好友"),
                        "lastSeen": user_data.get("lastSeen", 0) # 如果沒有 lastSeen 則預設為 0
                    })
        leaderboard_search_services_logger.info(f"成功查詢了 {len(friend_statuses)} 個好友的狀態。")
        return friend_statuses
    except Exception as e:
        leaderboard_search_services_logger.error(f"查詢好友狀態時發生錯誤: {e}", exc_info=True)
        return []
