...
def initialize_firebase_for_script():
    ...
    if not firebase_admin._apps: 
        ...
        if firebase_credentials_json_env:
            ...
            # 尋找相對於 backend 資料夾的金鑰
            key_path_in_backend = os.path.join(os.path.dirname(__file__), '..', SERVICE_ACCOUNT_KEY_PATH)
            if os.path.exists(key_path_in_backend):
                try:
                    cred = credentials.Certificate(key_path_in_backend)
                    script_logger.info(f"成功從本地檔案 '{key_path_in_backend}' 創建憑證物件。")
                except Exception as e:
                    script_logger.error(f"從本地檔案 '{key_path_in_backend}' 創建 Firebase 憑證物件失敗: {e}", exc_info=True)
                    cred = None
            else:
                script_logger.warning(f"本地金鑰檔案 '{key_path_in_backend}' 不存在。")
            # --- 修改結束 ---
