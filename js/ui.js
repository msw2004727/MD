...
function setupLeaderboardTableHeaders(tableId, headersConfig) {
    ...
    headersConfig.forEach(header => {
        ...
        th.classList.add('sortable'); // Make all headers sortable visually
...
function updateLeaderboardTable(tableType, data) {
    ...
    data.forEach((item, index) => {
        ...
        row.dataset.monsterId = item.id; // 新增：為 tr 加上怪獸ID，以便事件監聽
        ...
        if (tableType === 'monster') {
            ...
            nicknameLink.href = '#';
            nicknameLink.className = 'leaderboard-monster-link'; // 加上這個class
