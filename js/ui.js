...
function initializeDOMElements() {
    DOMElements = {
        ...
        farmTableWrapper: document.getElementById('farm-table-wrapper'), // 修改: 直接獲取外層容器
...
function renderMonsterFarm() {
    const farmWrapper = DOMElements.farmTableWrapper;
    ...
    const headersHtml = `
        <div class="farm-header-grid" id="farm-headers">
    ...
    if (!sortedMonsters || sortedMonsters.length === 0) {
        monstersHtml = `<p class="text-center text-sm text-[var(--text-secondary)] py-4 col-span-full">農場空空如也，快去組合怪獸吧！</p>`;
    } else {
        const rarityMap = {'普通':'common', '稀有':'rare', '菁英':'elite', '傳奇':'legendary', '神話':'mythical'};
        monstersHtml = sortedMonsters.map(monster => {
            ...
            if (isTraining) cultivateBtnClasses += ' secondary'; else cultivateBtnClasses += ' warning';

            return `
                <div class="farm-monster-item ${isDeployed ? 'selected' : ''}" data-monster-id="${monster.id}">
                    <div class="farm-col farm-col-battle">
                        <button class="farm-battle-btn button ${battleButtonClass}" title="${battleButtonTitle}">${battleButtonIcon}</button>
                    </div>
                    ...
                    <div class="farm-col farm-col-actions">
                        <button class="${cultivateBtnClasses}" title="${isTraining ? '召回修煉' : '開始修煉'}" ${isDeployed ? 'disabled' : ''}>${cultivateBtnText}</button>
    ...
    farmWrapper.innerHTML = `<div id="farm-content-scroller">${headersHtml}<div id="farmed-monsters-list">${monstersHtml}</div></div>`;

    // Re-attach event listeners after innerHTML overwrite
    const farmContentScroller = farmWrapper.querySelector('#farm-content-scroller');
    if (farmContentScroller) {
        farmContentScroller.addEventListener('click', (e) => {
            const monsterItem = e.target.closest('.farm-monster-item');
            if (!monsterItem) return;
            const monsterId = monsterItem.dataset.monsterId;

            if (e.target.closest('.farm-battle-btn')) {
                e.stopPropagation();
                handleDeployMonsterClick(monsterId);
            } else if (e.target.closest('.farm-monster-name-link')) {
                e.preventDefault();
                e.stopPropagation();
            ...
            } else if (e.target.closest('.farm-monster-cultivate-btn')) {
                e.stopPropagation();
                const monster = gameState.playerData.farmedMonsters.find(m => m.id === monsterId);
                if (monster?.farmStatus?.isTraining) {
                    handleEndCultivationClick(e, monsterId, monster.farmStatus.trainingStartTime, monster.farmStatus.trainingDuration);
                } else {
                    handleCultivateMonsterClick(e, monsterId);
                }
            } else if (e.target.closest('.farm-monster-release-btn')) {
                e.stopPropagation();
                handleReleaseMonsterClick(e, monsterId);
            }
        });
    }

    const newFarmHeaders = farmWrapper.querySelector('#farm-headers');
    if (newFarmHeaders) {
        newFarmHeaders.addEventListener('click', (event) => {
            ...
            gameState.farmSortConfig = { key: sortKey, order: newSortOrder };
            renderMonsterFarm();
        });
    }
