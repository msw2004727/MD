...
function updateMonsterInfoModal(monster, gameConfigs, ownerPlayerData = null) {
    ...
    // 獲取稱號加成
    let titleBuffs = {};
    if (ownerPlayerData && ownerPlayerData.playerStats) {
        const equippedId = ownerPlayerData.playerStats.equipped_title_id;
        if (equippedId) {
            const equippedTitle = (ownerPlayerData.playerStats.titles || []).find(t => t.id === equippedId);
    ...
    // 建立一個同時處理修煉加成(綠字)和稱號加成(紅字)的函式
    const getGainAndBuffHtml = (statName) => {
        const cultivationGain = monster.cultivation_gains?.[statName] || 0;
        const titleGain = titleBuffs[statName] || 0;
        
        let html = '';
        if (cultivationGain > 0) {
            html += ` <span style="color: var(--success-color); font-size: 0.9em; margin-left: 4px;">+${cultivationGain}</span>`;
        }
        if (titleGain > 0) {
            html += ` <span style="color: var(--danger-color); font-size: 0.9em; margin-left: 4px;">+${titleGain}</span>`;
        }
        return html;
    };
    // --- 修改結束 ---
        ...
        <div class="details-grid-rearranged">
            <div class="details-column-left" style="display: flex; flex-direction: column;">
                <div class="details-section" style="margin-bottom: 0.5rem;">
                    ...
                    <div class="details-item"><span class="details-label">HP:</span> <span class="details-value">${monster.hp}/${monster.initial_max_hp}${getGainAndBuffHtml('hp')}</span></div>
                    <div class="details-item"><span class="details-label">MP:</span> <span class="details-value">${monster.mp}/${monster.initial_max_mp}${getGainAndBuffHtml('mp')}</span></div>
                    <div class="details-item"><span class="details-label">攻擊:</span> <span class="details-value">${monster.attack}${getGainAndBuffHtml('attack')}</span></div>
                    <div class="details-item"><span class="details-label">防禦:</span> <span class="details-value">${monster.defense}${getGainAndBuffHtml('defense')}</span></div>
                    <div class="details-item"><span class="details-label">速度:</span> <span class="details-value">${monster.speed}${getGainAndBuffHtml('speed')}</span></div>
                    <div class="details-item"><span class="details-label">爆擊率:</span> <span class="details-value">${monster.crit}%${getGainAndBuffHtml('crit')}</span></div>
