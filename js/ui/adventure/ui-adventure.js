                ...
                card.addEventListener('click', () => {
                    ...
                    if (selectedIndex > -1) {
                        ...
                        if (selectedMonsters.length < 3) {
                            ...
                            const deselectedId = selectedMonters.shift(); 
...
async function initializeAdventureUI() {
    ...
    try {
        ...
        adventureTabContent.innerHTML = `<p class="text-center text-lg text-[var(--text-secondary)] py-10" style="color: var(--danger-color);">錯誤：無法載��冒險島資料。<br>${error.message}</p>`;
    }
}